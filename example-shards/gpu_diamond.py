...
def frames(layer):
    ...
    prog = window.ctx.program(
        ...
        fragment_shader='''
            ...
            void main() {
                float efx = 1.0;
                float efy = 1.0;
                float ooome = 1.0 / (1.0 - abs(eccentricity));
                if (eccentricity < 0.0) {
                    efx = ooome;
                } else {
                    efy = ooome;
                }
                f_color = texture(palette, vec2(scale * (efx * abs(uv.x - center.x - 0.5) + efy * abs(uv.y - center.y - 0.5)) + offset, 0.0));
            }
        ''',
    )
    ...
    while True:
        ...
        # set the uniforms
        prog['center'].value = (layer.variable_manager.variables["centerX"].value, layer.variable_manager.variables["centerY"].value)
        prog['scale'].value = layer.variable_manager.variables["scale"].value
        prog['offset'].value = timewarp.local() % 1.0
        prog['eccentricity'].value = layer.variable_manager.variables["eccentricity"].value
